...
def show_class_slot(user, parent_widget, action_frame):
    ...
    ttk.Label(left_frame, text="Subject subject", style="CustomLabel.TLabel").grid(row=0, column=0, padx=10, pady=5,
                                                                                sticky="ns")
    ...
    columns = ("Subject subject", "Subject Name", "Year", "Assigned", "Day", "Start Time", "End Time")
    ...
    if conn:
        cursor = conn.cursor()
        try:
            # Perform a JOIN operation to combine data from both tables based on the subject subject
            cursor.execute("""
                SELECT subject.subject, subject.name, subject.year, subject.assigned, time_slot.day,
                       time_slot.start_time, time_slot.end_time
                FROM subject
                JOIN time_slot ON subject.subject = time_slot.subject  -- Corrected table name here
            """)
            rows = cursor.fetchall()
            for row in rows:
...
def save_class_slot(subject_subject_var, subject_name_var, year_var, assigned_var, day_var, start_time_var,
                    end_time_var, class_listbox):
    try:
        subject_subject = subject_subject_var.get()
        ...
        save_subject_details(subject_subject, subject_name, year, assigned)
        # Save time slot details
        save_time_slot_details(subject_subject, day, start_time, end_time)
        # Insert the new item into the Treeview
        class_listbox.insert("", "end",
                             values=(subject_subject, subject_name, year, assigned, day, start_time, end_time))

        messagebox.showinfo("Success", "Class slot saved successfully.")
        subject_subject_var.set('')
...
def save_subject_details(subject, subject_name, year, assigned):
    # Save subject details to subject table in the database
    conn = db_connect()
    if conn:
        cursor = conn.cursor()
        query = "INSERT INTO subject (subject, name, year, assigned) VALUES (%s, %s, %s, %s)"
        cursor.execute(query, (subject, subject_name, year, assigned))
        conn.commit()
        conn.close()


def save_time_slot_details(subject, day, start_time, end_time):
    ...
    if conn:
        ...
        cursor.execute(query, (subject, day, start_time, end_time))
        conn.commit()
        conn.close()
...
def delete_selected_data(class_listbox):
    ...
    if conn:
        try:
            ...
            cursor.execute("DELETE FROM subject WHERE subject = %s", (subject_code,))
            conn.commit()
        ...
        try:
            cursor.execute("""
                SELECT subject.subject, subject.name, subject.year, subject.assigned, time_slot.day,
                       time_slot.start_time, time_slot.end_time
                FROM subject
                JOIN time_subject ON subject.subject = time_slot.code
            """)
            rows = cursor.fetchall()
            for row in rows:
                class_listbox.insert("", "end", values=row)
